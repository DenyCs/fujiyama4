        ...
        <div class="mt-8 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 shadow-sm dark:shadow-none">
            ...
            @if ($cart->items->isEmpty())
                <div class="text-neutral-500 dark:text-neutral-400 text-sm py-4 text-center border border-dashed border-neutral-300 dark:border-neutral-700 rounded-lg">
                    Belum ada menu dipilih. <a href="{{ route('client.menu') }}" class="text-orange-500 hover:text-orange-400 underline">Pilih menu dulu</a> jika ingin pre-order.
                </div>
            @else
                <div class="space-y-3">
                    @foreach ($cart->items as $item)
                        <div class="flex items-center justify-between bg-neutral-100 dark:bg-neutral-800 rounded-lg p-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-neutral-500 dark:text-neutral-400 text-xs">{{ $item->qty }}x — Rp {{ number_format($item->menu->price, 0, ',', '.') }}</p>
                            </div>
                    ...
                    <a href="{{ route('cart.index') }}" class="block text-center text-sm text-orange-500 hover:text-orange-400 mt-1">
                        Edit keranjang →
                    </a>
                </div>
            @endif
