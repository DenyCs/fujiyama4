                ...
                <div class="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
                    ...
                    <div class="group relative bg-neutral-900/80 backdrop-blur-sm border border-neutral-800 rounded-2xl p-4 text-left hover:border-orange-600/50 transition-all duration-300 hover:-translate-y-1">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h4 class="text-sm font-bold text-white truncate">{{ $menu->name }}</h4>
                                <p class="text-xs text-neutral-500 truncate">{{ $menu->category->name ?? '' }}</p>
                            </div>
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="group relative bg-neutral-900/80 border border-neutral-800 rounded-2xl overflow-hidden hover:border-orange-600/50 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-600/5">
                    ...
                    <!-- Content -->
                    <div class="p-5">
                        <h3 class="text-lg font-bold text-white mb-2 group-hover:text-orange-400 transition-colors">{{ $menu->name }}</h3>
                        <p class="text-sm text-neutral-500 line-clamp-2 mb-4 leading-relaxed">{{ $menu->description ?: 'Lezat, autentik, dan dibuat dengan bahan-bahan premium.' }}</p>

                        <div class="flex items-center justify-between">
                            <span class="text-xl font-bold text-orange-400">Rp {{ number_format($menu->price, 0, ',', '.') }}</span>
                            <button type="button"
                ...
                <div class="group relative bg-neutral-900/80 border border-neutral-800 rounded-2xl p-6 hover:border-orange-600/50 transition-all duration-300 hover:-translate-y-2 text-center">
                    ...
                    <div class="flex flex-wrap justify-center gap-2">
                        @foreach($category->menus->take(3) as $menu)
                            <span class="px-2.5 py-1 bg-neutral-800 text-xs text-neutral-400 rounded-full">{{ $menu->name }}</span>
                        @endforeach
                    </div>
                </div>
                @empty
                ...
                <div class="group relative bg-neutral-800 rounded-2xl overflow-hidden border border-neutral-700/50 hover:border-orange-500/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-orange-500/10">
                    <div class="aspect-[16/10] overflow-hidden">
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="inline-block px-3 py-1 text-xs font-bold rounded-full {{ $event->isActive() ? 'bg-orange-500/90 text-white' : 'bg-neutral-600/80 text-neutral-300' }}">
                    ...
                    <div class="p-6">
                        ...
                        <h3 class="font-extrabold text-white text-lg group-hover:text-orange-400 transition-colors mb-2">{{ $event->title }}</h3>
                        <p class="text-sm text-neutral-400 line-clamp-2">{{ $event->description }}</p>
                    </div>
                </div>
                @endforeach
            </div>
